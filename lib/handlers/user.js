...
_users.post = function(data, headers, callback){
  if(data.clientId)
    _userCtrl.createClient(data, callback);
  else
     if (data.phone){
        _userCtrl.createBuyer(data,callback);
      }
      else {
        ...
        _userCtrl.getOne(id, function(err, userData){
          if(!err && userData){
            let checks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
              if (_userData.role== "comprador"){
                _//el usuario renuncia a sus regalos y cierra la cuenta 
                _giftCtrl.delete(userData.username);
              }
              //si es cliente verificar que no tenga regalos sin cobrar con usuarios activos
              else {
                if _userCtrl(userData.activeUsers==0 && userData.activeGift ==0){
                  _userCtrl.delete(id, function(err){
                  if(!err){
                    if(checks.length > 0){
                  _   users.deleteChecks(checks, callback);
                     }else
                  callback(200);
                  }else
